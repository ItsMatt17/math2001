...
example {x y : ℝ} (h : y = 2 * x + 1) : x < y / 2 ∨ x > y / 2 := by
  have h2 : (x = (y - 1) / 2) := by addarith[h]
  left
  calc
    x = (y - 1) / 2 := by rw[h2]
    _ < (y - 1) / 2 + 1 / 2 := by extra
    _ = (y) / 2 := by ring
...
example {a b : ℝ} (hab : a ^ 2 + 2 * b ^ 2 = 3 * a * b) : a = b ∨ a = 2 * b := by
  have h2 : (a ^ 2 - 3 * a * b + 2 * b ^ 2 = 0) := by addarith[hab]
  have h3 :=
    calc
    (a - 1 * b) * (a - 2 * b) = a ^ 2 - 3 * a * b + 2 * b ^ 2 := by ring
    _ = 0 := by rw[h2]
...
example {t : ℝ} (ht : t ^ 3 = t ^ 2) : t = 1 ∨ t = 0 := by
  ...
  obtain h4 | h4 := eq_zero_or_eq_zero_of_mul_eq_zero h2

  · cancel 2 at h4
    right
    exact h4

  · have h5 :t = 1 := by addarith[h4]
    left
    exact h5

  
example {n : ℕ} : n ^ 2 ≠ 7 := by
  have h1 := le_or_succ_le n 2
  obtain h2 | h2 := h1
