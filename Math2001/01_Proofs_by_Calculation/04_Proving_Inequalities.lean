...
example {u v x y A B : ℝ} (h1 : 0 < A) (h2 : A ≤ 1) (h3 : 1 ≤ B) (h4 : x ≤ B)
  ...
  calc
    ...
    _ ≤ A * B + A * B + A * v := by rel [h8, h9] -- Why doesn't this become < ?
...
example {x : ℤ} (hx : x ≥ 9) : x ^ 3 - 8 * x ^ 2 + 2 * x ≥ 3 :=
  calc
    ...
    _ ≥ 9 * (9 + 2) := by rel[hx]
    _ = 99 := by ring
    _ ≥ 3 := by numbers -- WHY IS THIS GE??? Like I know you need that to solve the problem, but its not true?


example {n : ℤ} (hn : n ≥ 10) : n ^ 4 - 2 * n ^ 2 > 3 * n ^ 3 :=
  calc
    n ^ 4 - 2 * n ^ 2 = n * n * (n * n - 2) := by ring
...
example {x : ℚ} : x ^ 2 - 2 * x ≥ -1 :=
  calc
    ...
    _ ≥ -1 := by extra -- this works bc -1 <= (x - 1) ^ 2 - 1 (recall n <= m^2 + n)
