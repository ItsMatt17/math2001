...
example : {n : ℤ | Even n} = {a : ℤ | a ≡ 6 [ZMOD 2]} := by
  have h: 0 ≡ 6 [ZMOD 2] :=
    calc
      0 ≡ 0 [ZMOD 2] := by numbers
      _ ≡ 0 + 2 * 3 [ZMOD 2] := by extra
      _ ≡ 6 [ZMOD 2] := by numbers
  ...
  · obtain ⟨k, hk⟩ := h1
    calc
      a = 2 * k := by rw[hk]
      _ ≡ 2 * k [ZMOD 2] := by extra
      _ ≡ 0 [ZMOD 2] := by extra
      _ ≡ 6 [ZMOD 2] := by rel[h]
  · intro h1
    obtain ⟨k, hk⟩ := h1
    have hk: a = 2 * k + 6 := by addarith[hk]
    use (k + 3)
    rw[hk]
    ring
...
example : {k : ℤ | 7 ∣ 9 * k} = {l : ℤ | 7 ∣ l} := by
  ...
  · obtain ⟨k, hk⟩ := h
    use 4 * a - 3 * k
    calc
      a = 4 * (7 * a) - 3 * (9 * a) := by ring
      _ = 4 * (7 * a) - 3 * (7 * k) := by rw[← hk]
      _ = 7 * (4 * a - 3 * k) := by ring
  · obtain ⟨k, hk⟩ := h
    use 9 * k
    calc
